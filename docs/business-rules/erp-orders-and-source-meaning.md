...
| `MasterData`, spreadsheet `1ZL6cEwydC0cWSGP2I92uILn1ixILr_qAeDfDfD6F214` | the Master Data / Styles grid (`dam.designflow.app/styles`) |
...
| 21 Quantity | `lineQty` (order quantity; per-SKU `orderQty` also available) | 100% |
| 22 Case Pack | not on the order line — item grain via `/itemDetails` `cartonQty` | see note |
| 24 Ship To | the same routing code's destination part | see note |
...
7,948 / LA 2,964 / NJ 548 / SAVANNAH 67):
...
- **ColdLion differentiates the two; the Google OrderList does not.** The ERP carries distinct
  ...
  CONTRACTUAL SAMPLE / DAVID SAMPLE / C STOCK / STOCK / …) contains **no DDP value at all**, so
  DDP orders are labelled POE+NJ on the sheet. The automation's Order Type derived from the ERP
  routing code is therefore *more correct than the manual sheet column it replaces*.
...
- **FOB point is a production-order detail (Albert, 2026-09-17), and `/prodtracking` is where
  ...
  `prodTypeCode` (`FOBCHINA`, `FOBINDIA`, `FOBUSA`). Near-term, FOB Ship To (the specific
  Ningbo/Qingdao/Xiamen/Shanghai port) stays human or derives from the factory/vendor;
