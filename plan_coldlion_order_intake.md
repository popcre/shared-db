...
**Revised:** 2026-09-17 twice. (1) Qwen audit of v1 — verdict REJECT; record:
[`.ai/reviews/qwen-order-intake-plan-audit-5a2fd8b6ac9b199946cc14b3895d00b870e0746a04180c4f5ef58073e9fe6c4f.md`](.ai/reviews/qwen-order-intake-plan-audit-5a2fd8b6ac9b199946cc14b3895d00b870e0746a04180c4f5ef58073e9fe6c4f.md).
(2) Grok 4.6 audit of v2 — verdict "not ready" (blocking + major findings F1–F22, 11
contradiction claims); record:
[`.ai/reviews/grok-order-intake-plan-audit-v2-20260918T003208Z-368754.md`](.ai/reviews/grok-order-intake-plan-audit-v2-20260918T003208Z-368754.md)
...
record: [`.ai/reviews/muse-order-intake-plan-audit-v3-20260918T011029Z-606747-3631.md`](.ai/reviews/muse-order-intake-plan-audit-v3-20260918T011029Z-606747-3631.md).
...
**Handoff entry:** [`HANDOFF.d/2026-09-17T1440Z-edge-dev-zcode-order-intake-plan.md`](HANDOFF.d/2026-09-17T1440Z-edge-dev-zcode-order-intake-plan.md)
...
section *How a new order enters the system (OrderList intake)*. ⚠️ **That section, this plan's
`AGENTS.md` registration, and the `application-map.md` routing row are uncommitted working-tree
edits as of 2026-09-17 and do not exist on `main`.** Step 0 of this plan is to land them: they
merge **before or with** this plan, never after. If you are reading this plan on a checkout
where that section is missing, stop — you are on `main` without the authority, and every §8
locked decision is unverifiable.

## STATUS

| Step | State | Evidence |
|---|---|---|
| 0. Land the business-rules + plan docs (docs-only PR, merges first) | ⬜ open | — |
...
A fresh session starts at **Step 0**, after reading
[`docs/business-rules/erp-orders-and-source-meaning.md`](docs/business-rules/erp-orders-and-source-meaning.md)
(the intake section) and this whole file. All rows except C0 (✓ ruled) are open as of
2026-09-17.
...
- **Repo:** `u2giants/shared-db` (canonical; this repo). Consumer repos mirror it read-only.
...
- **Existing sync infrastructure:** `tools/coldlion-landing/*.mjs` (Node ESM) run by GitHub
  ...
  prepack detail → prod-details in one job (`:104-119`; prod-details must run after history); manual backfills exist as
  ...
  is a literal in the workflow env (`coldlion-landing-sync.yml:80,92`), and
...
- **Governance machinery the migration must use (details in §9 A0):** db-work issues + author
  lanes + centrally reserved 14-digit versions (`AGENTS.md:732-736`), the mandatory
  `.github/live-proofs/<work-issue>.sql` probe (`AGENTS.md:135-143`), and the
...
8. **Every field Adam types is available on `/orderHistory`** except the FOB origin port and
   ...
   rows), Quantity `lineQty` (a PARENT LINE TOTAL, repeated on every exploded component —
...
10. Endpoint mechanics: paged envelope, **undocumented page-size cap 200** (loop until
    `last=true`); `companyCode`, `fromDate`, `toDate` are **required** params (a
    a `salesOrderNo`-only query without dates returns HTTP 400 — `salesOrderNo` is reliable only combined
...
- **Tests in a `tests/` subdirectory.** `tools-offline-tests.yml:155` globs `tools/*.test.mjs`
...
- Grants follow the **actual sealed-history pattern** (`20260905105038:780-782`,
  `20260905014719:232-242`): `enable row level security` on every new table, `revoke all …
  from public, anon, authenticated`, **and `grant all … to service_role`** — every coldlion
  landing table grants service_role, and `20260905042713` is only a `plm`-scoped revoke of four
  privilege bits, not authority for a colder grants policy. The live-proof probe asserts RLS
  on, no public/anon/authenticated grants, and service_role `arwd`.
...
- Exact staging-table shape — mirror the `order_history_line`/`_component` projection minus
  ...
  row and nothing is ever updated in place; the run FKs (`first_seen_run`/`last_seen_run`)
...
**Step 0 — land the authority (merges FIRST, two PRs).** The documents-only merge path
treats `plan_*.md` and `AGENTS.md` as agent rulebooks with a strict grammar
(`scripts/lib/documents-only-change.mjs`: an `AGENTS.md` hunk is admissible only as a
list/table row whose link label literally equals the target filename — and this plan's
`AGENTS.md` registration is an instruction paragraph). So:
(a) **Docs-only PR:** the 2026-09-17 business-rules edits (`erp-orders-and-source-meaning.md`,
`application-map.md`, `master-data-access.md`) plus the `HANDOFF.d/` entry.
(b) **Guarded-merge PR:** `plan_coldlion_order_intake.md` plus its `AGENTS.md` paragraph
(or shrink the AGENTS hunk to a bare `- [plan_coldlion_order_intake.md](plan_coldlion_order_intake.md)`
row to keep it on the docs-only path — implementer's choice).
PR (a) merges before any code session starts; PR (b) before or with the code. Verify with
...
- **A0 (prerequisite — before any migration file is opened).** Open the `db-work` issue with
  this exact `db-work-scope` block (the grammar of `docs/agents/section-4-anti-collision-rules.md:186-227`
...
application_return_to: u2giants/shared-db
priority: 100
depends_on:
writes:
...
```
  ...
  isolated worktree>" --objects "<the six objects above>"`. Use **only** the reserved
  14-digit version in the filename; never derive a timestamp (`AGENTS.md:732-736`, `:1593`).
  Read `docs/agents/section-4-anti-collision-rules.md` in full first (`AGENTS.md:721-724`).
- **A1. New migration** `supabase/migrations/<reserved-version>_coldlion_order_intake_staging.sql`
  ...
  - `coldlion.routing_code_map` — curated decode: `code`, `description`, `order_type`,
    `ship_to`, `is_poe_ddp_family`, `notes`; seeded with the full observed vocabulary from the
    business-rules routing table.
  ...
  - *Verification gate:* the same PR commits `.github/live-proofs/<work-issue>.sql` — one
    read-only `WITH` returning one row with a boolean aliased `passed` (all six relations
    exist; `coldlion.window_ledger` row count unchanged; RLS on with the sealed-history grant
    pattern of §8 on the new tables) — because `scripts/check-live-proof-probe.mjs` refuses the merge without it
    (`AGENTS.md:135-143`). Offline: `Database Contract Tests` green, plus a new
...
- **A2. Verify item case-pack landing (verify-only, no migration):** confirm
  ...
  sync). No projection extension is needed; both are landed and projected (§5).
...
- **B1. New entry point** `tools/coldlion-landing/order-intake.mjs` (logic in
  ...
  - Detect: insert into `coldlion.intake_new_order` every `salesOrderNo` in the staged range
    ...
    loaded, and the very first live-proof week would find nothing.
...
- **C2. Canonical writer** `tools/coldlion-landing/order-intake-write.mjs`: for each
  ...
  0. **Resolve grain and version before mapping any field.** The winning version of every
     ...
     then `component_source_hash`, per `(line_id, sub_item_no, sub_label_code)` — the same
     ...
     components. **Any variation in any projected field outside the hash and run FKs
     quarantines the order** rather than picking a version silently — the settled rule
     that differing projections are separate versions, never merged, is honoured by asserting
     the winner is unambiguous. The component ordinal for `source_id` determinism = order of
  ...
  2. **Create** the placeholder header `COLDLION-SO-<salesOrderNo>` with
     ...
     the writer SQL itself. Customer resolution:
     `core.company_source_ref` (`source_system='coldlion'`, `source_table='customers'`,
     `source_id = customerCode`) → `customer_id` → `core.customer.id` FIRST;
...
- All are auto-discovered by the `FILES=(tools/*.test.mjs)` glob (`tools-offline-tests.yml:155`)
...
- Database: GitHub secrets — exactly two — `SUPABASE_DB_URL_PRODUCTION` (pooler URL for
  ...
  in the workflow env (`coldlion-landing-sync.yml:81`), and `proveTarget()` refuses mismatched
