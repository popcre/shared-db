...
| B2. Admit route `self-service-additive` in `QUEUE_ROUTES`/`ROUTES_BY_WORK_TYPE`/`evaluateAdmission`/expand paths; excluded from orchestrator refill | ⬜ open | 2026-09-17 | `--queue-audit` prints a separate self-service section and does not refill from it; a claim through the route succeeds and its objects lock; sync test green. |
| B3. Classifier enforced at merge time (pre-lock step in the guarded merge workflow) + write down the already-existing merge-dispatch right | ⬜ open | 2026-09-17 | Guard refuses a deliberately out-of-boundary fixture before any merge lock is taken; dispatch-right decision recorded in §8. |
