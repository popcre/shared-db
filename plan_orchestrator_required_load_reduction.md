...
| A3. Ship read-only scheduled `Queue Hygiene Report` (write-stripped subcommand + `author-lane-abandonment-audit.yml` shape) | 🟡 landed in review | 2026-09-17 | Subcommand `--queue-hygiene-report` + `hygieneReportIo` (every mutation hook throws) + `queue-hygiene-report.yml` (daily 04:23 UTC + dispatch, read-only permissions) + `scripts/queue-hygiene-report.test.mjs` (6 tests, wired into `tools-offline-tests.yml`). Live smoke run exited 0 on the backed-up queue. Gate: one green `workflow_dispatch` run after merge — flip to ✅ then. |
