...
export function verifyGitEvidence({ contract, report, prBaseSha, prHeadSha }, io) {
  ...
  if (JSON.stringify(actualFiles) !== JSON.stringify(reportedFiles)) {
    const missing = actualFiles.filter((file) => !reportedFiles.includes(file))
    ...
    throw new GitEvidenceError(`reported files_changed does not match Git: Git changed [${actualFiles.join(', ')}] but .agent/completion.json files_changed lists [${reportedFiles.join(', ')}]; add to the report [${missing.join(', ')}], remove from the report [${extra.join(', ')}]`)
  }
