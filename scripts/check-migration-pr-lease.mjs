...
export function gatherPrInput(env=process.env){
  ...
  return {claims:issues.filter(x=>!x.pull_request&&(x.labels??[]).some(l=>(typeof l==='string'?l:l?.name)==='db-claim')).map(x=>({number:x.number,body:x.body})),branch:pr.head.ref,files,reservationExists:(version)=>{try{return Boolean(json(['api',`repos/${REPO}/git/ref/db-claims/${version}`])?.object?.sha)}catch{return false}}}
}
