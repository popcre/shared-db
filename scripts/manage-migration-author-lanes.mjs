...
import { assertDrawPromptContract, collectCrossPrObjectCollisions, collectProtectedSourceCollisions, preDrawHandoffChecks } from './lib/reviewer-draw-readiness.mjs'
...
// accept already-fetched reads so the whole readiness result costs the same
// GitHub requests the two guards always cost on their own.
//
// A read that fails proceeds, exactly as the guards above always have: a
// transport fault never becomes a reviewer refusal. A definite bad fact
// refuses by name, and a refusal never reads as an approval.
function preDrawRead(fn,arg){
  if(typeof fn!=='function')return undefined
  try{return fn(arg)}catch{return undefined}
}
export function assertReviewerDrawHandoff(o,io=githubIo){
  // 1. The carried brief, before anything is read or consumed (#2998 fix 1).
  ...
  return preDrawHandoffChecks({pr:Number(o.pr),headSha:o.headSha,rows,live},io)
}
...
export const githubIo = {
  ...
  // without them skips the check, while the real CLI always runs it. Both
  // collect functions skip their gather (zero API calls) unless this pull
  // request actually edits a protected source or adds a migration.
  protectedSourceCollisions(pr,rows){return collectProtectedSourceCollisions({repo:REPO,pr,rows})},
  crossPrObjectCollisions(pr,rows){return collectCrossPrObjectCollisions({repo:REPO,pr,rows})},
...
export function main(argv, now = new Date(), io = githubIo) {
  try {
    ...
    if(o.replaceFailedReviewer){assertReviewerDrawHandoff(o,io);const result=replaceFailedReviewer({...o,slot:o.reviewSlot!==undefined?Number(o.reviewSlot):1,admissionOptions:io.enforceAdmission===true?o:null},io);console.log(JSON.stringify(result,null,2));return 0}
    ...
    if(o.assignReviewer){assertReviewerDrawHandoff(o,io);console.log(JSON.stringify(assignWithMutexRetry({issue:o.issue,pr:o.pr,headSha:o.headSha,slot:o.reviewSlot!==undefined?Number(o.reviewSlot):1,reviewerAllowlist:o.reviewerAllowlist,admissionOptions:io.enforceAdmission===true?o:null},io),null,2));return 0}
