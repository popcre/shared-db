...
class RepositoryIdentityTests(unittest.TestCase):
    ...
    def test_remote_forms(self):
        for url in ("https://github.com/popcre/shared-db.git", "https://x-access-token:abc@github.com/popcre/shared-db",
    ...
    def test_env_origin_disagreement_refused_case_insensitive_agreement_allowed(self):
        with self.assertRaises(ri.RepositoryIdentityError):
            ri.resolve_repository_identity(env={"GITHUB_REPOSITORY": "popcre/shared-db"}, read_origin=origin("git@github.com:u2giants/shared-db.git"))
        self.assertEqual(ri.resolve_repository_identity(env={"GITHUB_REPOSITORY": "PopCre/shared-db"}, read_origin=origin("git@github.com:popcre/shared-db.git")), "PopCre/shared-db")
