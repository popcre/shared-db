-- Issue #3175: DesignFlow sync is retired; its historical evidence remains.
-- derived-from: 20260905043023
-- Only two existing detector bodies change. No function is invoked here.
...
begin
  ...
  -- Retired DesignFlow run metadata is historical only; no freshness/status gate.
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'designflow_lane', jsonb_build_object('status', 'retired', 'required', false)
    )
  )
  returning id into v_health_run_id;
...
comment on function plm.check_taxonomy_sync_health(interval, jsonb) is
  'Phase 6 health; retired DesignFlow runtime evidence is informational, never required. Expected baseline read from plm.taxonomy_baseline_pin (was compiled-in constants in 20260726180000), and only when a baseline is ACTIVE on this database -- otherwise it refuses with a failed sync_run and a warning alert rather than passing or tripping. Evaluates latest NON-DRILL observation for the 14-day gate. force_fail records a distinct drill alert without overwriting daily evidence.';
...
declare
  ...
  -- Compatibility NOT NULL field: true means not required, NOT a successful run.
  v_designflow_ok boolean := true;
...
begin
  ...
  -- DesignFlow runtime lane retired: keep its evidence, but never require a run.
