...
_FIXES = (
    ...
    ("Framed Glass Shadowbox", r"\b(?:printed glass )?shadowbox frame\b|\bprinted glass shadowbox\b"),
    ...
    ("Hanging Wall Art", r"\b(?:wool )?fabric embroidered hanging wall art\b"),
...
def _mixed_distinct_forms(title: str) -> bool:
    ...
    for raw_clause in re.split(r"\s*(?:,|\+|&|\band\b)\s*", title):
        clause = re.sub(r"\bwrtng\s+dsk\b", "writing desk", normalize(DIMENSION.sub(" ", raw_clause)))
        form = ""
...
def read_product_type(description: object) -> dict[str, str]:
    ...
    if len(parts) > 1 and not any(pattern.search(normalized_title) for _, pattern in PRODUCT_PATTERNS):
        ...
        if re.fullmatch(r"(?:(?:printed|stretched|framed) )?canvas(?: set)?", next_clause):
            ...
            title += " " + parts[1]
    artwork_marker = re.search(r"\b(?:artwork|illustration|graphic|image|scene)\b", title, re.I)
    if artwork_marker:
        ...
        caption_medium = re.search(
            r"\b(?:canvas|mdf|wood|glass|paper|fabric|foil|glitter|metallic|"
    ...
    if any(entry[3] in {"Plaque", "Door Hanger"} for entry in matches):
    ...
    if re.search(r"\bframed\b.{0,25}\bmdf\b.{0,80}\bposter\b", text) \
        ...
        return result
    if product == "Framed Art" and match.group() in {"framed mdf", "frm mdf"} \
    ...
    for _, _, order, other, found in matches:
        if found.start() < match.end() and match.start() < found.end():
            ...
            if joined_product and not (product == "Plaque" and other in {"Wall Hook", "Hook"}) \
